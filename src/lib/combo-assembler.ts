...
export interface ShadowModeComparison {
  ceComboCount: number;
  legacyComboCount: number;
  ceTierDistribution: Record<ConfidenceTier, number>;
  legacyTierDistribution: Record<ConfidenceTier, number>;
  itemOverlapPercent: number;
  anyLowInCeWhenHighExists: boolean;
}

/**
 * Compare CE-driven combos with legacy MatchingEngine combos.
 * For shadow mode logging.
 */
export function compareCombosForShadowMode(
  ceCombos: AssembledCombo[],
  legacyCombos: { items: { id: string }[] }[]
): ShadowModeComparison {
  // CE tier distribution
  const ceTierDist: Record<ConfidenceTier, number> = { HIGH: 0, MEDIUM: 0, LOW: 0 };
  for (const combo of ceCombos) {
    ceTierDist[combo.tierFloor]++;
  }

  // Legacy tier distribution (we don't have tiers, so just count)
  const legacyTierDist: Record<ConfidenceTier, number> = {
    HIGH: legacyCombos.length, // Assume all were "considered valid"
    MEDIUM: 0,
    LOW: 0
  };

  // Item overlap in top 3 combos
  const ceItems = new Set<string>();
  ceCombos.slice(0, 3).forEach(c => c.candidates.forEach(cand => ceItems.add(cand.itemId)));

  const legacyItems = new Set<string>();
  legacyCombos.slice(0, 3).forEach(c => c.items?.forEach(item => legacyItems.add(item.id)));

  let overlap = 0;
  for (const id of ceItems) {
    if (legacyItems.has(id)) overlap++;
  }
  const overlapPercent = ceItems.size > 0 ? (overlap / ceItems.size) * 100 : 0;

  // Regression check: any LOW in CE when HIGH exists?
  const hasHighInCE = ceCombos.some(c => c.tierFloor === 'HIGH');
  const hasLowInCE = ceCombos.some(c => c.tierFloor === 'LOW');
  const anyLowWhenHighExists = hasHighInCE && hasLowInCE;

  return {
    ceComboCount: ceCombos.length,
    legacyComboCount: legacyCombos.length,
    ceTierDistribution: ceTierDist,
    legacyTierDistribution: legacyTierDist,
    itemOverlapPercent: overlapPercent,
    anyLowInCeWhenHighExists: anyLowWhenHighExists,
  };
}

/**
 * Log shadow mode comparison (console for now).
 */
export function logShadowModeComparison(comparison: ShadowModeComparison): void {
  console.log('[ComboAssembler:Shadow]', JSON.stringify(comparison, null, 2));
}
